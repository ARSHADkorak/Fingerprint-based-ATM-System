...
def ViewBalance():
    ...
    output = ""
    con = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root123', database = 'atm',charset='utf8')
    with con:
        cur = con.cursor()
        ...
        with con:
            ...
            for row in rows:
                if row[0] == user and password == row[1]:
                    in_file = open("C:/Users/kings/Downloads/Fingerprint-based-ATM-System-main/Fingerprint-based-ATM-System-main/static/users/"+user+".png", "rb")
...
def WithdrawAction():
    if request.method == 'POST':
        ...
        if total > withdraw:
            ...
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root123', database = 'atm',charset='utf8')
...
def SignupAction():
    if request.method == 'POST':
        ...
        fingerprint_data = request.form['fingerprintData']  # Retrieve fingerprint data from the form
        ...
        with con:
            ...
            if cur.fetchone() is not None:
                status = f"Username '{user}' already exists"
        ...
        with con:
            cur = con.cursor()
            student_sql_query = "INSERT INTO users (username, password, contact_no, emailid, address, gender, fingerprint_data) VALUES (%s, %s, %s, %s, %s, %s, %s)"
            cur.execute(student_sql_query, (user, password, phone, email, address, gender, fingerprint_data))
            con.commit()
